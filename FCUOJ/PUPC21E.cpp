...
signed main(){
    ...
    for(int i=1;i<=t;i++){
        ...
        for(auto f:rect){
            vw[V_path[f.first].second] = vw[V_path[f.first].first]+f.second.second;
            hw[H_path[f.first].second] = hw[H_path[f.first].first]+f.second.first;
            cout << f.first << " " << hw[H_path[f.first].first] << " " << vw[V_path[f.first].first] << "\n";
        }
    }
}